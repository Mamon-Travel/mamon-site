import { getHizmetler } from '../services/hizmetlerService'

export async function getNavigation(): Promise<TNavigationItem[]> {
  // Hizmetler API'sinden dinamik veri çek
  const hizmetler = await getHizmetler()
  
  // Hizmetleri navigation formatına çevir
  const hizmetNavigationItems: TNavigationItem[] = hizmetler.map((hizmet) => ({
    id: hizmet.id.toString(),
    href: hizmet.url,
    name: hizmet.ad,
  }))

  return [
    ...hizmetNavigationItems,
    {
      id: '5',
      href: '/',
      name: 'Templates',
    ...
    },
    {
      ...
      name: 'Travelers',
      type: 'dropdown',
      children: hizmetNavigationItems,
    },
  ]
}
